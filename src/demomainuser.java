...
public class demomainuser extends JFrame implements ActionListener{
   ...
   // bg : label to add images in it
    ...
    JButton [] add_button = new JButton[size];
    JButton [] min_button = new JButton[size];    
    JLabel [] add_but = new JLabel[size] ;
  ...
  JButton ltalabat = new JButton("Talabat");
    ...
    ImageIcon add_img = new ImageIcon("Designs/Main for user/Assets/1x/add button1.png");
    ImageIcon min_img = new ImageIcon("Designs/Main for user/Assets/1x/min button.png");
    public demomainuser ()
    {
      ...
      for(int i = 0 ; i < Talabat.owners.get(RestaurantMain.knowRestaurat).restaurant.meals.size(); i++)
      {
         ...
         /* meal[i] = new JLabel();
          ...
          if(imgxbound <= 500)
          {
           ...
           // add (add button )
           add_button[i] = new JButton();
           add_button[i].setIcon(add_img);
           add_button[i].setBounds(imgxbound+15, imgybound+145, 24 , 24);
           add_button[i].setBorder(null);
            add_button[i].setBorderPainted(false);
           add_button[i].setFocusPainted(false);
           add_button[i].setContentAreaFilled(false);
           meal_part.add(add_button[i]);    
           
           
           //min button
           min_button[i] = new JButton();
           min_button[i].setIcon(min_img);
           min_button[i].setBounds(imgxbound+2, imgybound+162, 50 , 50);
           min_button[i].setBorder(null);
            min_button[i].setBorderPainted(false);
           min_button[i].setFocusPainted(false);
           min_button[i].setContentAreaFilled(false);
           meal_part.add(min_button[i]);
